 <!-- Main modal -->
 <div id="edit{{ $itemGalih->bukuId }}" tabindex="-1" aria-hidden="true"
         ...
         <div class="relative bg-white w-[40rem] rounded-lg shadow :bg-gray-700">
             ...
             <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t :border-gray-600">
                 ...
                 <button type="button"
                     ...
                     data-modal-hide="edit{{ $itemGalih->bukuId }}">
             ...
             <div class="p-4 md:p-5 flex flex-col gap-4 ">
                 <form action="{{ route('adminBuku.update', $itemGalih->bukuId) }}" enctype="multipart/form-data"
                     method="POST">
                     @csrf
                     @method('put')
                     <div>
                         <label for="email" class="block mb-2 text-sm font-medium text-gray-900 :text-white">Judul
                             buku</label>
                         <input type="text" value="{{ $itemGalih->judul }}" name="judul" id="email"
                     ...
                     <div>
                         ...
                         <input type="text" name="penulis" value="{{ $itemGalih->penulis }}" id="email"
                             class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 :bg-gray-600 :border-gray-500 :placeholder-gray-400 :text-white"
                             placeholder="Judul buku (contoh harimau malaya )" required />
                     </div>
                     <div class="flex gap-2 w-full">
                         <div class="flex flex-col flex-1">
                             ...
                             <input type="text" name="penerbit" value="{{ $itemGalih->penerbit }}" id="email"
                                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 :bg-gray-600 :border-gray-500 :placeholder-gray-400 :text-white"
                                 required />
                         </div>
                         <div class="flex flex-col flex-1">
                             ...
                             <select name="tahun_terbit"
                                 class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 :bg-gray-600 :border-gray-500 :placeholder-gray-400 :text-white"
                                 required id="">
                                 @for ($i = 1889; $i <= \Carbon\Carbon::now()->format('Y'); $i++)
                                     <option {{ $itemGalih->tahun_terbit === $i ? 'selected' : '' }}
                                         value="{{ $i }}">{{ $i }}</option>
                                 @endfor
                             </select>
                         </div>
                     ...
                     <div class="flex flex-col">
                         ...
                         <div class="flex mt-3 flex-wrap gap-2">
                             ...
                             @php
                                 $kategoriArray = [];
                                 foreach ($itemGalih->kategori as $key) {
                                     $kategoriArray[] = $key->pivot->id_kategori;
                                 }
                                 //  @dd($kategoriArray);
                             @endphp
                             ...
                             @foreach ($kategori as $galihKategori)
                                 <div class="mt-4">
                                     <input {{ in_array($galihKategori->kategoriId, $kategoriArray) ? 'checked' : '' }}
                                         id="kategoriedit{{ $galihKategori->kategoriId . $itemGalih->bukuId }}"
                                         class="peer sr-only" type="checkbox" name="kategori[]"
                                         value="{{ $galihKategori->kategoriId }}">
                                     <label
                                         class="bg-blue-500 cursor-pointer text-white py-2 px-4 peer-checked:bg-blue-900"
                                         for="kategoriedit{{ $galihKategori->kategoriId . $itemGalih->bukuId }}">{{ $galihKategori->nama_kategori }}</label>
                                 </div>
                             @endforeach
                     ...
                     <div class="">
                         ...
                         <input type="text" value="{{ $itemGalih->sinopsis }}" name="sinopsis" id="email"
                             class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 :bg-gray-600 :border-gray-500 :placeholder-gray-400 :text-white"
